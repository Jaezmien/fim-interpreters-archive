...
I learned the sum of everything from 1 to 100 to get a number. (start of a new method)
    ...
    As long as the current count was no more than 100: (Start a conditional statement that loops while the current count is less than or equal to 100)
        P.S. I would add the current count to the sum. (Increment the sum by the value held in the current count)

        P.S. I changed the method to this one because the old docs are confusing. ~M
        the sum became the current count plus the sum. (Add the current count to the sum)
